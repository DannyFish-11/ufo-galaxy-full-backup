...
class PolarDBGraphDB(BaseGraphDB):
    ...
    def _create_graph(self):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                # 使用参数化查询防止SQL注入
                schema_name = f"{self.db_name}_graph"
                # 验证schema名称只包含合法字符
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                for ext_name, ext_desc in extensions:
                    try:
                        # 验证扩展名只包含合法字符
                        if not re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', ext_name):
                            raise ValueError(f"Invalid extension name: {ext_name}")
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                if graph_exists:
                    ...
                    # 验证数据库名称
                    graph_name = f"{self.db_name}_graph"
        ...
        for label_name in valid_rel_types:
            ...
            try:
                ...
                with conn.cursor() as cursor:
                    ...
                    cursor.execute(f"select create_elabel($graph_name, $label_name);")
    ...
    def get_edges(
        ...
        # Add type filter
        if type != "ANY":
            where_clause += f" AND type(r) = $rel_type"
